 ...
 * @since 3.0.0
 */
class FSBatch implements Iterable<ICommitterRequest> {

    private CachedStreamFactory streamFactory;
    private final Path dir;
    private final long max;

    // -1 (or anything <= 0) means whole dir
    // a max... means only up to max.
    public FSBatch(CachedStreamFactory streamFactory, Path dir, long max) {
        super();
    ...
    @Override
    public CountingIterator<ICommitterRequest> iterator() {
        try {
    ...
    public void move(Path toDir) throws IOException {
        Path targetDir = toDir.resolve(dir.getFileName());
        Iterator<Path> it = zipIterator();
        while (it.hasNext()) {
    ...
    private Iterator<Path> zipIterator() throws IOException {
        Iterator<Path> it = FSQueueUtil.findZipFiles(dir).iterator();
        if (max > 0) {
